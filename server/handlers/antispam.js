...
function initFirstSpamClassifier(classifier) {
  ...
  classifier.addDocument('Присоединяйтесь к нам в Facebook >>>', 'spam'); // 1
                                                                          // =
                                                                          // spam
  classifier.addDocument(
      'натуральный комплекс веществ, состоящий из природных компонентов >>>',
      'spam'
  ); // 1 = spam
  classifier.addDocument('для суставов из уникальных пант канадского марала',
      'spam'
  ); // 1 = spam
  classifier.addDocument('Черная и Красная икра доставим домой и в офис >>>',
      'spam'
  ); // 1 = spam
  classifier.addDocument(
      'Венера Мухамедовна Абдуллаева Директор ООО «СЕРТ Академия» в г. Казань' +
      ' Телефон: (843) 514-76-32, (843) 524-72-19, 8(917)281-28-19 ' +
      'russia@cert-academy.org',
      'spam'
  ); // 1 = spam
  ...
  classifier.addDocument(
      'Не используйте изображение во фрейме. Иначе не будет работать. Можно решить проксируя все запросы через свой сервер. ',
      'ok'
  ); // 0 - not spam
...
module.exports.getClassifications = function(text) {
  ...
  return classifier.getClassifications(t);
};
